...
def test(test_length: int) -> None:
    ...
    if test_length == 1:
        ...
        guess: int = int(input(f"Alright, {player}, I am thinking of one number from 1 to 9. What is your prediction? "))
        if guess == number:
    ...
    if test_length == 2:
        ...
        guess_1: int = int(input(f"Alright, {player}, I am thinking of two numbers from 1 to 9. What is your prediction for the first number? "))
        guess_2: int = int(input("And your prediction for the second number? "))
...
def path_2(points: int) -> int:
    ...
    enter_path_2: str = str(input("Since you're here, are you ready for some insight into my test? Yes or No? "))
    while enter_path_2 != "Yes":
        enter_path_2: str = str(input(f"Please don't waste my time, {player}. Shall we proceed to the test information? Yes or No? "))
...
def main() -> None:
    ...
    while path == 2 or path == 3:
        ...
        if path == 2:
            ...
            print(f"You have {points} correct guesses so far, {player}. Keep up the practice.")
        if path == 3:
            test_length: int = int(input(f"How many numbers would you like to guess this time, {player}? 1 or 2? "))
            test(test_length)
